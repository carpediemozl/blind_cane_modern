...
class TrafficPipeline:
    def __init__(self, modelPath, labelMap, syncNN=True):
        ...
        # [CRITICAL REFACTOR] Spatial calculator setup for older API
        spatialLocationCalculator = self.pipeline.create(dai.node.SpatialLocationCalculator)
        # Input node for sending ROI configurations from the host (RPi)
        ...
        stereo.setDepthAlign(dai.CameraBoardSocket.RGB) # Critical alignment fix
        ...
        detectionNetwork.setBlobPath(self.modelPath)
        ...
        # [CRITICAL REFACTOR] Linking for host-side configuration
        stereo.depth.link(spatialLocationCalculator.inputDepth)
        xinSpatialCalcConfig.out.link(spatialLocationCalculator.inputConfig) # The correct input port
    ...
    def run(self):
        with dai.Device(self.pipeline) as device:
            ...
            while True:
                ...
                detections = inNN.detections
                spatialData = [] # [FIX] Initialize as empty list

                # [CRITICAL LOGIC FIX]
                # Only try to get spatial data if we have sent a config in this frame
                if len(detections) > 0:
                    # 1. Create and send the ROI config to the device
                    ...
                    # 2. Now that we've sent a config, we can expect a result
                    inSpatialData = spatialDataQueue.get()
                    spatialData = inSpatialData.getSpatialLocations()

                # Associate detections with their spatial data
                if len(detections) == len(spatialData):
                    for i, detection in enumerate(detections):
                        ...
                        try: label = self.labelMap[detection.label]
                        except: label = detection.label

                        color = red if label == 'red' else green
